...
The barbell and dumbbell in the weight picker are procedural Three.js models, not mesh files. They sit under one large live weight with no chrome around them. A pair of bells is still one per-hand load: a muted "per hand" gloss sits after the unit. Single has no qualifier. The live weight's chevron opens a glass listbox of bar weights — not a segmented pill. Gym references: urethane circular dumbbell, competition bumper plates on a chrome Olympic bar. Equipment (dumbbell / barbell / custom) is a horizontal pager. The open exercise card is the swipe field except the log cluster (reps, Log, drop): a finger drag tracks 1:1 as a hinge-card slide (translate plus a shallow rotateY, scale, and fade), rubber-bands at the ends, and settles with ease-out-expo. Reverse, cancel, and a lost pointer snap from the finger's current page, not the down-point. The pager slot is the tallest page so Log and reps never reflow. Vertical intent still scrolls the page or steps the dumbbell rack. Arrow keys on the focused card change method too. On barbell, plate chips sit immediately under the bar.
