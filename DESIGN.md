...
The barbell and dumbbell in the weight picker are procedural Three.js models, not mesh files. They sit under one large live weight with no chrome around them. A pair of bells is still one per-hand load: a muted "per hand" gloss sits after the unit. Single has no qualifier. The live weight's chevron opens a glass listbox of bar weights — not a segmented pill. Gym references: urethane circular dumbbell, competition bumper plates on a chrome Olympic bar. Equipment (dumbbell / barbell / custom) is a horizontal pager. The open exercise card is the swipe field except Log, drop, and the reps control: a finger drag tracks 1:1 as a hinge-card slide (translate plus a shallow rotateY, scale, and fade), rubber-bands at the ends, and settles with ease-out-expo. Reverse, cancel, and a lost pointer snap from the finger's current page, not the down-point. Reps are one compact inline companion under the pager, not a second labeled stepper. The pager height tracks the active page, including during the swipe, so reps and Log stay one gap below the instrument on every mode. Vertical intent still scrolls the page or steps the dumbbell rack. Arrow keys on the focused card change method too. On barbell, plate chips sit immediately under the bar.

- **Camera:** fixed perspective, parked on the object's midline and looking slightly down the shaft so a disc reads as an ellipse (~0.23 of plate height on the bar, ~0.20 on the dumbbell's outer face). Distance is the pose; only FOV reacts to the element's size. The bar is fitted to a constant 45 cm envelope, so adding or removing plates never changes its size on screen. Pointer input nudges elevation by at most 2°. On the dumbbell, a vertical drag steps through rack sizes (heavier up); a tap toggles pair (two bells) vs single (one). Horizontal paging is owned by the open exercise card, not the 3D object. Outside the dumbbell's vertical step, vertical scroll still owns the page. `prefers-reduced-motion` holds the rest pose and crossfades equipment instead of sliding. A tap on a plate still removes it.
...
- **Motion:** plates settle onto the sleeve in 160ms ease-out-expo. Equipment pages track the finger as a hinge-card slide, then settle in ~380ms ease-out-expo; reverse, cancel, and a lost pointer snap from the visual page. During the slide, live WebGL is frozen to a still so the CSS 3D hinge does not blank the instruments. Reduced motion skips the slide. Under `prefers-reduced-motion`, plates appear in place and tilt is off.
